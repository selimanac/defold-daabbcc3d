...
function collision.insert_aabb(x, y, width, height, collision_bit)
	...
	return daabbcc3d.insert_aabb(aabb_group_id, x, y, width, height, collision_bit)
end

function collision.insert_gameobject(go_url, width, height, collision_bit)
	collision_bit = collision_bit and collision_bit or nil
	return daabbcc3d.insert_gameobject(aabb_group_id, go_url, width, height, collision_bit)
end
...
function collision.query_aabb(x, y, width, height, is_mask, get_manifold)
	...
	return daabbcc3d.query_aabb(aabb_group_id, x, y, width, height, mask_bits, get_manifold)
end
...
function collision.query_aabb_sort(x, y, width, height, is_mask, get_manifold)
	local mask_bits = is_mask and selected_mask_bits or default_mask_bits
	get_manifold    = get_manifold and get_manifold or nil
	return daabbcc3d.query_aabb_sort(aabb_group_id, x, y, width, height, mask_bits, get_manifold)
end

function collision.raycast(ray_start, ray_end, is_mask, get_manifold)
	local mask_bits = is_mask and selected_mask_bits or default_mask_bits
	get_manifold    = get_manifold and get_manifold or nil
	return daabbcc3d.raycast(aabb_group_id, ray_start.x, ray_start.y, ray_end.x, ray_end.y, mask_bits, get_manifold)
end

function collision.raycast_sort(ray_start, ray_end, is_mask, get_manifold)
	local mask_bits = is_mask and selected_mask_bits or default_mask_bits
	get_manifold    = get_manifold and get_manifold or nil
	return daabbcc3d.raycast_sort(aabb_group_id, ray_start.x, ray_start.y, ray_end.x, ray_end.y, mask_bits, get_manifold)
end

function collision.update_aabb(aabb)
	daabbcc3d.update_aabb(aabb_group_id, aabb.aabb_id, aabb.position.x, aabb.position.y, aabb.size.width, aabb.size.height)
end
