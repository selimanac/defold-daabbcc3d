...
#define B2_TREE_STACK_SIZE 1024
    ...
    void b2DynamicTree_Destroy(b2DynamicTree* tree)
    {
        ...
        b2Free(tree->leafCenters, tree->rebuildCapacity * sizeof(b2Vec3));
    ...
    {
        b2Vec3            centerD = b2AABB_Center(boxD);
        ...
        while (nodes[index].height > 0)
        {
            ...
            if (lowerCost1 == lowerCost2 && leaf1 == false)
            {
                ...
                b2Vec3 d1 = b2Sub(b2AABB_Center(box1), centerD);
                b2Vec3 d2 = b2Sub(b2AABB_Center(box2), centerD);
    ...
    int b2DynamicTree_GetByteCount(const b2DynamicTree* tree)
    {
        ...
        tree->rebuildCapacity * (sizeof(int32_t) + sizeof(b2AABB) + sizeof(b2Vec3) + sizeof(int32_t));
    ...
    bool RayIntersectsAABB(const b2Vec3& rayStart, const b2Vec3& rayEnd, const b2AABB& aabb, float& outDistance)
    {
        ...
        float  tmin = 0.0f;     // Start from zero
        float  tmax = 1.0f;     // Since rayEnd is finite, max distance is 1 (scaled)
        float  epsilon = 1e-6f; // Small threshold to prevent division errors
        ...
        else if (rayStart.x < aabb.lowerBound.x || rayStart.x > aabb.upperBound.x)
        {
            return false; // Ray is outside and parallel to X axis
        }
        ...
        else if (rayStart.y < aabb.lowerBound.y || rayStart.y > aabb.upperBound.y)
        {
            return false; // Ray is outside and parallel to Y axis
        }
        ...
        else if (rayStart.z < aabb.lowerBound.z || rayStart.z > aabb.upperBound.z)
        {
            return false; // Ray is outside and parallel to Z axis
        }

        // Valid intersection check
        if (tmax >= tmin && tmin >= 0.0f && tmin <= 1.0f)
        {
            // Compute the actual intersection distance
            outDistance = sqrtf(dir.x * dir.x + dir.y * dir.y + dir.z * dir.z) * tmin;
            return true;
        }
    ...
    b2TreeStats b2DynamicTree_RayCast(const b2DynamicTree* tree, const b2RayCastInput* input, uint64_t maskBits, b2TreeRayCastCallbackFcn* callback, void* context)
    {
        ...
        b2Vec3 p1 = input->origin;
        b2Vec3 d = input->translation;
        ...
        b2Vec3 p2 = b2MulAdd(p1, maxFraction, d);
        ...
        while (stackCount > 0)
        {
            ...
            b2Vec3 c = b2AABB_Center(nodeAABB);
            b2Vec3 h = b2AABB_Extents(nodeAABB);
            ...
            if (b2IsLeaf(node))
            {
                float hitDistance;
                // To @selim -> this is added
                if (RayIntersectsAABB(p1, p2, node->aabb, hitDistance))
                {
                    subInput.maxFraction = maxFraction;
                    subInput.distance = hitDistance;
            ...
            else
            {
                ...
                if (stackCount < B2_TREE_STACK_SIZE - 1)
                {
                    b2Vec3 c1 = b2AABB_Center(nodes[node->child1].aabb);
                    b2Vec3 c2 = b2AABB_Center(nodes[node->child2].aabb);
...
#define B2_TREE_HEURISTIC 0
    ...
    static int32_t b2PartitionMid(int32_t* indices, b2Vec3* centers, int32_t count)
    {
        // Handle trivial case
        if (count <= 2)
        {
        ...
        b2Vec3 lowerBound = centers[0];
        b2Vec3 upperBound = centers[0];
        ...
        b2Vec3 d = b2Sub(upperBound, lowerBound);
        b2Vec3 c = { 0.5f * (lowerBound.x + upperBound.x), 0.5f * (lowerBound.y + upperBound.y) };
...
#define B2_BIN_COUNT 64
    ...
    static int32_t b2PartitionSAH(int32_t* indices, int32_t* binIndices, b2AABB* boxes, int32_t count)
    {
        ...
        b2Vec3      center = b2AABB_Center(boxes[0]);
        b2AABB      centroidAABB;
        ...
        b2Vec3 d = b2Sub(centroidAABB.upperBound, centroidAABB.lowerBound);
        ...
        {
            bins[i].aabb.lowerBound = (b2Vec3) { FLT_MAX, FLT_MAX };
            bins[i].aabb.upperBound = (b2Vec3) { -FLT_MAX, -FLT_MAX };
            bins[i].count = 0;
        }
        ...
        {
            b2Vec3  c = b2AABB_Center(boxes[i]);
...
#if B2_TREE_HEURISTIC == 0
        b2Vec3* leafCenters = tree->leafCenters;
#else
...
#if B2_TREE_HEURISTIC == 0
            b2Free(tree->leafCenters, tree->rebuildCapacity * sizeof(b2Vec3));
            tree->leafCenters = (b2Vec3*)b2Alloc(newCapacity * sizeof(b2Vec3));
#else
            b2Free(tree->leafBoxes, tree->rebuildCapacity * sizeof(b2AABB));
